...
public class LoanWritePlatformServiceJpaRepositoryImpl implements LoanWritePlatformService {
   ...
   // @Autowired
    ...
    public LoanWritePlatformServiceJpaRepositoryImpl(final PlatformSecurityContext context,
            ...
            final FromJsonHelper fromJsonHelper,final ReadWriteNonCoreDataService readWriteNonCoreDataService,
            final MailProcessingService mailProcessingService, final Environment env, final LoanTransactionToRepaymentScheduleMappingRepository loanTransactionToRepaymentScheduleMappingRepository) {
    ...
    public CommandProcessingResult disburseLoan(final Long loanId, final JsonCommand command, Boolean isAccountTransfer) {
        ...
        final BigDecimal amt = command.bigDecimalValueOfParameterNamed("transactionAmount");
...
//            String templateParamString = "templateParameters[A]="+amountDisbursed.toString()+"&templateParameters[B]="+actualDisbursementDate.toString("dd/MM/yyyy");
    ...
    public CommandProcessingResult makeLoanRepayment(final Long loanId, final JsonCommand command, final boolean isRecoveryRepayment) {
        ...
        final String txnExternalId = command.stringValueOfParameterNamedAllowingNull("externalId");
        final Boolean isExculdeCharges = command.booleanObjectValueOfParameterNamed("excludecharges");
        final boolean isRepaymentCharge = command.booleanObjectValueOfParameterNamed("isRepaymentCharge");
       ...
       // this.loanTransactionToRepaymentScheduleMappingRepository.findByLoanTransactionToRepaymentScheduleMapping(loanTransactions);
        ...
        boolean isAccountTransfer = false;
        final Type typeOfMap = new TypeToken<Map<String, String>>() {}.getType();//new
        final Map<String, String> dataParams = this.fromJsonHelper.extractDataMap(typeOfMap, command.json());//new
      ...
      //code to send mail start  
        ...
        final String mailId = this.readWriteNonCoreDataService.getMailIdForColumn(loanId,mailDataTableName,mailDataColunName);
      
      
        Boolean isActiveMailRepaymentReceipt = Boolean.valueOf(dataParams.get("repaymentmail"));
        Boolean isActiveMailClosure = Boolean.valueOf(dataParams.get("loanclosuremail"));
        Boolean isMailSendRepaymentReceipt =false;
        Boolean isMailSendClosure =false;
        if(isActiveMailRepaymentReceipt){//repayment Receipt
        ...
        queryParams.add("R_amount",String.valueOf(transactionAmount) );
        queryParams.add("R_rdate",String.valueOf(transactionDate) );
        final File filename=this.readWriteNonCoreDataService.getFile( queryParams, env.getProperty("mail.report.name.repaymentreceipt"), errorLog,pdf);
     
       
	            if(CommonMethodsUtil.isNotBlank(mailId)&&CommonMethodsUtil.isNotBlank(filename)) {
	               
	                MailDataVO maildata =new MailDataVO("repaymentreceipt", mailId, "",env.getProperty("mail.repayment.subject.name"),filename.getAbsolutePath(),env.getProperty("mail.repayment.content"));
	                isMailSendRepaymentReceipt=this.mailProcessingService.sendMailFile(maildata);
        ...
        LoanTransaction transaction=this.loanAccountDomainService.makeRepayment(loan, commandProcessingResultBuilder, transactionDate, transactionAmount,
                paymentDetail, noteText, txnExternalId, isRecoveryRepayment, isAccountTransfer, holidayDetailDto, isHolidayValidationDone
                ,isMailSendRepaymentReceipt,isMailSendClosure,isExculdeCharges, isRepaymentCharge);
        Date closeddate=loan.getClosedOnDate();
        
        
        
        if(isActiveMailClosure){//single Disbursement will do multi Disbursement
           	if(CommonMethodsUtil.isNotBlank(closeddate)&& loan.getLoanStatus()==600){

        if(CommonMethodsUtil.isNotBlank(loan.getAccountNumber())&&CommonMethodsUtil.isNotBlank(loan.client().getDisplayName())&&CommonMethodsUtil.isNotBlank(transactionAmount)) {
        	String templateParameterString = loan.client().getDisplayName()+","+loan.getAccountNumber().toString()+","+loan.getCurrencyCode()+","+transactionAmount.toString()+","+transactionDate.toString("dd/MM/yyyy");
          	 MailDataVO maildata = new MailDataVO("normalclose", mailId, templateParameterString,"loanclosurecontent","","");
          	isMailSendClosure=this.mailProcessingService.sendMail(maildata);
          	transaction.setCloseSend(isMailSendClosure);
            }
           	}}
...
//        String templateParamString = "templateParameters[A]="+loan.client().getDisplayName()+"&templateParameters[B]="+loan.getAccountNumber()+"&templateParameters[C]="+loan.getCurrencyCode()+"&templateParameters[D]="+transactionAmount.toString()+"&templateParameters[E]="+transactionDate.toString("dd/MM/yyyy")+"&templateParameters[F]="+loan.getSummary().getTotalOutstanding().toString()+"&templateParameters[G]="+loan.getCurrencyCode();
        	String templateParamString = "templateParameters[A]="+loan.client().getFirstname()+"&templateParameters[B]="+loan.getId().toString()+"&templateParameters[C]="+loan.getCurrencyCode()+"&templateParameters[D]="+transactionAmount.toString()+"&templateParameters[E]="+transactionDate.toString("dd/MM/yyyy");
        ...
        for (final SingleRepaymentCommand singleLoanRepaymentCommand : repaymentCommand) {
            if (singleLoanRepaymentCommand != null) {
                ...
                LoanTransaction loanTransaction = this.loanAccountDomainService.makeRepayment(loan, commandProcessingResultBuilder,
                        ...
                        isHolidayValidationDone,false,false, false,false);
...
//                	String templateParamString = "templateParameters[A]="+loan.getAccountNumber()+"&templateParameters[B]="+loan.getCurrencyCode()+"&templateParameters[C]="+singleLoanRepaymentCommand.getTransactionAmount().toString()+"&templateParameters[D]="+bulkRepaymentCommand.getTransactionDate().toString("dd/MM/yyyy");
    ...
    public CommandProcessingResult adjustLoanTransaction(final Long loanId, final Long transactionId, final JsonCommand command) {
      ...
      //code to send mail start  
        ...
        LoanTransaction newTransactionDetail = LoanTransaction.repayment(loan.getOffice(), transactionAmountAsMoney, paymentDetail,
                transactionDate, txnExternalId, DateUtils.getLocalDateTimeOfTenant(), currentUser,isMailSendRepaymentReceipt,isMailSendClosure);
        ...
        final ChangedTransactionDetail changedTransactionDetail = loan.adjustExistingTransaction(newTransactionDetail,
                ...
                scheduleGeneratorDTO, currentUser,false, false);
        
       //send close loan letter
        Date closeddate=loan.getClosedOnDate();
        if(isActiveMailClosure){
           	if(CommonMethodsUtil.isNotBlank(closeddate)&& loan.getLoanStatus()==600 &&closeddate.equals(transactionDate.toDate())){

        if(CommonMethodsUtil.isNotBlank(loan.getAccountNumber())&&CommonMethodsUtil.isNotBlank(loan.client().getDisplayName())&&CommonMethodsUtil.isNotBlank(transactionAmount)) {
        	String templateParameterString = loan.client().getDisplayName()+","+loan.getAccountNumber().toString()+","+loan.getCurrencyCode()+","+transactionAmount.toString()+","+transactionDate.toString("dd/MM/yyyy");
          	 MailDataVO maildata = new MailDataVO("normalclose", mailId, templateParameterString,"loanclosurecontent","","");
          	isMailSendClosure=this.mailProcessingService.sendMail(maildata);
          	newTransactionDetail.setCloseSend(isMailSendClosure);
            }
          }
    ...
    public CommandProcessingResult forecloseLoan(final Long loanId, final JsonCommand command) {
      ...
      //validation needed.
        ...
        final Boolean isActiveMailRepaymentReceipt = Boolean.valueOf(dataParams.get("repaymentmail"));
        final Boolean isActiveMailForeClosure = Boolean.valueOf(dataParams.get("loanclosuremail"));
        MultivaluedMap<String, String> queryParams = new  MultivaluedMapImpl();
      //changed by Ranjith
        final Map<String, Object> modifications = this.loanAccountDomainService.foreCloseLoan(loan, transactionDate, interestPortion, noteText
        		,isActiveMailRepaymentReceipt,isActiveMailForeClosure,queryParams);
  ...
  /*  public CommandProcessingResult repaymentCharge(final Long loanId, Long loanChargeId, final JsonCommand command, final boolean isChargeIdIncludedInJson) {
        ...
        loanTransaction = this.loanAccountDomainService.makeChargePayment(toLoanAccount, accountTransferDTO.getChargeId(),
                ...
                accountTransferDTO.getLoanInstallmentNumber(), false, false);
