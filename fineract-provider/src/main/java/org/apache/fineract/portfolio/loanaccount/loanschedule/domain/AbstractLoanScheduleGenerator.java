...
public abstract class AbstractLoanScheduleGenerator extends LoanDisbursementDetails implements LoanScheduleGenerator {
    ...
    private LoanScheduleModel generate(final MathContext mc, final LoanApplicationTerms loanApplicationTerms,
        ...
        final BigDecimal interestBrokenChargesDueAtTimeOfDisbursement = deriveTotalInterestBrokenChargesDueAtTimeOfDisbursement(loanApplicationTerms,mc,holidayDetailDTO);
        ...
        if (loanScheduleParams == null) {
            scheduleParams = LoanScheduleParams.createLoanScheduleParams(currency, Money.of(currency, chargesDueAtTimeOfDisbursement),Money.of(currency,interestBrokenChargesDueAtTimeOfDisbursement),
            ...
            scheduleParams = LoanScheduleParams
                    .createLoanScheduleParams(currency, Money.of(currency, chargesDueAtTimeOfDisbursement), Money.of(currency,interestBrokenChargesDueAtTimeOfDisbursement),
                            loanApplicationTerms.getExpectedDisbursementDate(), getPrincipalToBeScheduled(loanApplicationTerms),
                            loanScheduleParams);
        } else {
            scheduleParams = loanScheduleParams;
        ...
        if(!scheduleParams.isPartialUpdate()) {
            periods = createNewLoanScheduleListWithDisbursementDetails(numberOfRepayments,
                    loanApplicationTerms, chargesDueAtTimeOfDisbursement, interestBrokenChargesDueAtTimeOfDisbursement);
        }
        ...
        if (!scheduleParams.isPartialUpdate()) {
            ...
            if (loanApplicationTerms.isMultiDisburseLoan()) {
                ...
                BigDecimal disburseAmt = getDisbursementAmount(loanApplicationTerms, scheduleParams.getPeriodStartDate(), periods,
                        chargesDueAtTimeOfDisbursement,interestBrokenChargesDueAtTimeOfDisbursement, scheduleParams.getDisburseDetailMap(), scheduleParams.applyInterestRecalculation());
        ...
        while (!scheduleParams.getOutstandingBalance().isZero() || !scheduleParams.getDisburseDetailMap().isEmpty()) {
            ...
            if (loanApplicationTerms.isMultiDisburseLoan()) {
                updateBalanceBasedOnDisbursement(loanApplicationTerms, chargesDueAtTimeOfDisbursement,interestBrokenChargesDueAtTimeOfDisbursement, scheduleParams, periods,
                        scheduledDueDate);
            }
        ...
        return LoanScheduleModel.from(periods, applicationCurrency, scheduleParams.getLoanTermInDays(),
                ...
                scheduleParams.getTotalCumulativeInterest().getAmount(), scheduleParams.getTotalFeeChargesCharged().getAmount(),scheduleParams.getTotalInterestBrokenChargesCharged().getAmount(),
                        ...
                        if (applicableDate.isBefore(scheduledDueDate)) {
                            ...
                            if (unprocessed.isGreaterThanZero()) {
                                ...
                                if (!scheduleParams.getOutstandingBalance().isGreaterThan(unprocessed)
                                    ...
                                    if (!principalInterestForThisPeriod.interest()
                                        ...
                                        LoanTransaction loanTransaction = LoanTransaction.repayment(null, unprocessed, null,
                                                transactionDate, null, DateUtils.getLocalDateTimeOfTenant(), null,false,false);
                                ...
                                LoanTransaction loanTransaction = LoanTransaction.repayment(null, unprocessed, null, scheduledDueDate,
                                        null, DateUtils.getLocalDateTimeOfTenant(), null,false,false);
    ...
    private void updateBalanceBasedOnDisbursement(final LoanApplicationTerms loanApplicationTerms,
            final BigDecimal chargesDueAtTimeOfDisbursement, final BigDecimal interestBrokenChargesDueAtTimeOfDisbursement, LoanScheduleParams scheduleParams,
        ...
        for (Map.Entry<LocalDate, Money> disburseDetail : scheduleParams.getDisburseDetailMap().entrySet()) {
            if (disburseDetail.getKey().isAfter(scheduleParams.getPeriodStartDate()) && !disburseDetail.getKey().isAfter(scheduledDueDate)) {
                ...
                final LoanScheduleModelDisbursementPeriod disbursementPeriod = LoanScheduleModelDisbursementPeriod.disbursement(
                        disburseDetail.getKey(), disburseDetail.getValue(), chargesDueAtTimeOfDisbursement ,interestBrokenChargesDueAtTimeOfDisbursement);
  ...
  //changed by Ranjith
    private BigDecimal deriveTotalInterestBrokenChargesDueAtTimeOfDisbursement(final LoanApplicationTerms loanApplicationTerms,final MathContext mc,final HolidayDetailDTO holidayDetailDTO) {
        ...
        if(loanApplicationTerms.isMultiDisburseLoan()){
      	  ...
      	  BigDecimal interestBrokenChargeTrancheSum = BigDecimal.ZERO;
      	  for(DisbursementData disb:loanApplicationTerms.getDisbursementDatas()){
      		  interestBrokenChargeTranche= deriveTrancheInterestBrokenChargesDueAtTimeOfDisbursement(loanApplicationTerms,disb.amount(),disb.disbursementDate(),mc,holidayDetailDTO);
      		  disb.updateinterestBrokenChargeTranche(interestBrokenChargeTranche);
      		  interestBrokenChargeTrancheSum=interestBrokenChargeTrancheSum.add(interestBrokenChargeTranche);
      	  }
      	  interestBrokenchargesDueAtTimeOfDisbursement=interestBrokenChargeTrancheSum;
        }
        else{
        
         Money Amount =loanApplicationTerms.getTotalDisbursedAmountForInterestBroken();
         BigDecimal disbursementAmount=Amount.getAmount();
         BigDecimal interestBrokenDays=BigDecimal.ZERO;
        LocalDate interestChargedFromDate=loanApplicationTerms.getInterestChargedFromLocalDate();
        LocalDate disburesementDate=loanApplicationTerms.getExpectedDisbursementDate();
       ...
       if(loanApplicationTerms.getInterestRatePeriodFrequencyType().getValue()==3){
      	    BigDecimal  nominalInterestRate = loanApplicationTerms.getAnnualNominalInterestRate();
        ...
        if(daysInYear==1){
          ...
          else if(loanApplicationTerms.getInterestRatePeriodFrequencyType().getValue()==2){
      	     BigDecimal  monthInterestRate = loanApplicationTerms.getInterestRatePerPeriod();
      	     Integer daysInMonth=loanApplicationTerms.getDaysInMonthType().getValue();
          if(daysInMonth==1){
    ...
    //changed by Ranjith
    private BigDecimal deriveTrancheInterestBrokenChargesDueAtTimeOfDisbursement(LoanApplicationTerms loanApplicationTerms,BigDecimal disbursementAmount,LocalDate disburesementDate,final MathContext mc,final HolidayDetailDTO holidayDetailDTO) {
        BigDecimal interestBrokenchargesDueAtTimeOfDisbursement = BigDecimal.ZERO;
         BigDecimal interestBrokenDays=BigDecimal.ZERO;
        LocalDate interestChargedFromDate=loanApplicationTerms.getInterestChargedFromLocalDate();
          if(interestChargedFromDate!=null){
        	  if(disburesementDate.isBefore(interestChargedFromDate)){
        		  ...
        		  interestChargedFromDate=getNearestRepaymentDate(loanApplicationTerms,disburesementDate,holidayDetailDTO);
        		  if(interestChargedFromDate!=null){
        		  interestBrokenDays = BigDecimal.valueOf(Days.daysBetween(disburesementDate, interestChargedFromDate).getDays());
        		  }
        	  }
    ...
    public LocalDate getNearestRepaymentDate(LoanApplicationTerms loanApplicationTerms,LocalDate disburesementDate,final HolidayDetailDTO holidayDetailDTO) {
        
        LocalDate dueDate=new LocalDate();
        List<LocalDate> allRepaymentDate = this.scheduledDateGenerator.getAllRepaymentDates(loanApplicationTerms, holidayDetailDTO);
        List<LocalDate> dates = new ArrayList<LocalDate>();
        for (LocalDate repaymentDate : allRepaymentDate) {
            if (repaymentDate.isAfter(disburesementDate) || repaymentDate.isEqual(disburesementDate)) {
    ...
    private BigDecimal getDisbursementAmount(final LoanApplicationTerms loanApplicationTerms, LocalDate disbursementDate,
            final Collection<LoanScheduleModelPeriod> periods, final BigDecimal chargesDueAtTimeOfDisbursement,final BigDecimal interestBrokenchargesDueAtTimeOfDisbursement,
        ...
        for (DisbursementData disbursementData : loanApplicationTerms.getDisbursementDatas()) {
            if (disbursementData.disbursementDate().equals(disbursementDate)) {
                final LoanScheduleModelDisbursementPeriod disbursementPeriod = LoanScheduleModelDisbursementPeriod.disbursement(
                        disbursementData.disbursementDate(), Money.of(currency, disbursementData.amount()), chargesDueAtTimeOfDisbursement ,interestBrokenchargesDueAtTimeOfDisbursement);
                periods.add(disbursementPeriod);
    ...
    private Collection<LoanScheduleModelPeriod> createNewLoanScheduleListWithDisbursementDetails(final int numberOfRepayments,
            final LoanApplicationTerms loanApplicationTerms, final BigDecimal chargesDueAtTimeOfDisbursement ,final BigDecimal interestBrokenChargesDueAtTimeOfDisbursement ) {
    ...
    private Money calculateSpecificDueDateChargeWithPercentage(final Money principalDisbursed,
        ...
        // Changed by Mohan
        BigDecimal taxAmount = loanChargeAmt.multiply(loanCharge.taxPercentage()).divide(BigDecimal.valueOf(100));
        cumulative = cumulative.plus(loanChargeAmt).plus(taxAmount);
        return cumulative;
    }
    ...
    private Money calculateInstallmentCharge(final PrincipalInterest principalInterestForThisPeriod, Money cumulative,
      ...
      final MathContext mc = new MathContext(8, roundingMode);
        if (loanCharge.getChargeCalculation().isPercentageBased()) {
            ...
            // Changed by Ranjith
            BigDecimal loanChargeAmt = amount.multiply(loanCharge.getPercentage()).divide(BigDecimal.valueOf(100),mc.getRoundingMode());
            loanChargeAmt = loanChargeAmt.setScale(0, mc.getRoundingMode());         
         // Changed by Mohan
            BigDecimal taxAmount = loanChargeAmt.multiply(loanCharge.taxPercentage()).divide(BigDecimal.valueOf(100),mc.getRoundingMode());
            taxAmount = taxAmount.setScale(0, mc.getRoundingMode());
            cumulative = cumulative.plus(loanChargeAmt).plus(taxAmount);
        } else {
        	// Changed by Mohan
            BigDecimal taxAmount = loanCharge.amountOrPercentage().multiply(loanCharge.taxPercentage()).divide(BigDecimal.valueOf(100),mc.getRoundingMode());
            // Changed by Ranjith 
           	taxAmount= taxAmount.setScale(0, mc.getRoundingMode());  
           	BigDecimal loanChargeAmt = loanCharge.amountOrPercentage().setScale(0, mc.getRoundingMode());  
               cumulative = cumulative.plus(loanChargeAmt).plus(taxAmount);
        }
        return cumulative;
    }
    ...
    private LoanScheduleDTO rescheduleNextInstallments(final MathContext mc, final LoanApplicationTerms loanApplicationTerms,
        ...
        if (rescheduleFrom != null) {
            ...
            final BigDecimal interestBrokenChargesDueAtTimeOfDisbursement = deriveTotalChargesDueAtTimeOfDisbursement(loan.charges());
            ...
            Money totalInterestBrokenChargesCharged = principalToBeScheduled.zero().plus(interestBrokenChargesDueAtTimeOfDisbursement);
            ...
            if (loanApplicationTerms.isMultiDisburseLoan()) {
                ...
                BigDecimal disburseAmt = getDisbursementAmount(loanApplicationTerms, loanApplicationTerms.getExpectedDisbursementDate(),
                        periods, chargesDueAtTimeOfDisbursement, interestBrokenChargesDueAtTimeOfDisbursement, disburseDetailMap, true);
            ...
            totalRepaymentExpected = totalCumulativePrincipal.plus(totalCumulativeInterest).plus(totalFeeChargesCharged).plus(totalInterestBrokenChargesCharged) 
            ...
            if (!newRepaymentScheduleInstallments.isEmpty() && totalCumulativeInterest.isGreaterThanZero()) {
                ...
                loanScheduleParams = LoanScheduleParams.createLoanScheduleParamsForPartialUpdate(periodNumber, instalmentNumber,
                        ...
                        totalFeeChargesCharged, totalInterestBrokenChargesCharged, totalPenaltyChargesCharged, totalRepaymentExpected,
    ...
    public LoanRepaymentScheduleInstallment calculatePrepaymentAmount(final MonetaryCurrency currency, final LocalDate onDate,
        ...
        loanRepaymentScheduleTransactionProcessor.handleTransaction(loanApplicationTerms.getExpectedDisbursementDate(), loanTransactions,
                currency, loanScheduleDTO.getInstallments(), loan.charges(), false, null,false);
