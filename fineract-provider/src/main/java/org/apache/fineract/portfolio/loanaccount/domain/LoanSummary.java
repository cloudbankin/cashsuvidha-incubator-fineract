...
public final class LoanSummary {
    ...
    @Column(name = "total_interest_broken_due_at_disbursement_derived", scale = 6, precision = 19)
    private BigDecimal totalInterestBrokenChargesDueAtDisbursement;
    ...
    public void updateTotalInterestBrokenChargesDueAtDisbursement(final BigDecimal totalInterestBrokenChargesDueAtDisbursement) {

    	this.totalInterestBrokenChargesDueAtDisbursement = totalInterestBrokenChargesDueAtDisbursement;
    }
    ...
    public Money getTotalInterestBrokenChargesDueAtDisbursement(final MonetaryCurrency currency) {
         return Money.of(currency, this.totalInterestBrokenChargesDueAtDisbursement);
    }
    ...
    public void updateSummary(final Loan loan, final MonetaryCurrency currency, final Money principal,
        ...
        final Money totalInterestCharged = summaryWrapper.calculateTotalInterestCharged(repaymentScheduleInstallments, currency).plus( this.totalInterestBrokenChargesDueAtDisbursement);
        this.totalInterestCharged = totalInterestCharged.getAmount();
        BigDecimal disbursmentInterestMoney=BigDecimal.ZERO;
		//changed by Ranjith
        if(loan.getDisbursementDetails().size()>0){
	        for (LoanDisbursementDetails disb : loan.getDisbursementDetails()) {
	        	if(disb.getActualDisbursementDate()!=null){
		            if (Money.of(currency, this.totalInterestBrokenChargesDueAtDisbursement).isGreaterThanZero()) {
		            	disbursmentInterestMoney=disbursmentInterestMoney.add(disb.getInterestBrokenTranche());
		            }
	        	}
	        }
        }
        else{
        	disbursmentInterestMoney=this.totalInterestBrokenChargesDueAtDisbursement;
        }
        
        this.totalInterestRepaid = summaryWrapper.calculateTotalInterestRepaid(repaymentScheduleInstallments, currency).getAmount().add(disbursmentInterestMoney);
