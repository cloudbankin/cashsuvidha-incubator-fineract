...
public class Loan extends AbstractPersistableCustom<Long> {
    ...
    @Column(name = "amount_return", scale = 6, precision = 19,nullable = true)
    ...
    private LoanSummary updateSummaryWithTotalInterestBrokenChargesDueAtDisbursement(final BigDecimal interestBrokenChargesDueAtDisbursement) {
        if (this.summary != null) {
             this.summary.updateTotalInterestBrokenChargesDueAtDisbursement(interestBrokenChargesDueAtDisbursement);
         }
         return this.summary;
  }
    ...
    public void addLoanCharge(final LoanCharge loanCharge) {
        ...
        if (loanCharge.getChargeCalculation().isPercentageBased()) {
            ...
            if (loanCharge.isInstalmentFee()) {
                ...
                // Changed by Ranjith
                taxAmount = calculatePerInstallmentTaxAmount(loanCharge);
            } else if (loanCharge.isOverdueInstallmentCharge()) {
            	// Changed by Mohan
                totalChargeAmt = loanCharge.amountOutstanding().subtract(loanCharge.taxAmount());
            }
        } else {
            chargeAmt = loanCharge.amountOrPercentage();
        }   
        // Changed by Ranjith
    	if(loanCharge.getChargeCalculation().isPercentageBased() && loanCharge.isInstalmentFee()){
    		loanCharge.updateInstallmentFee(chargeAmt, loanCharge.getDueLocalDate(), amount, fetchNumberOfInstallmensAfterExceptions(), totalChargeAmt,taxAmount);
    	}
    	else{
    ...
    public ChangedTransactionDetail reprocessTransactions() {
        ...
        changedTransactionDetail = loanRepaymentScheduleTransactionProcessor.handleTransaction(getDisbursementDate(),
                allNonContraTransactionsPostDisbursement, getCurrency(), getRepaymentScheduleInstallments(), charges(), false, null,false);
    ...
    private void handleChargePaidTransaction(final LoanCharge charge, final LoanTransaction chargesPayment,
            final LoanLifecycleStateMachine loanLifecycleStateMachine, final Integer installmentNumber, final boolean isExcludeCharges, final Set<HabLoanRepaymentSchedule> habLoanRepaymentSchedule, final boolean isRepaymentCharge) {
        ...
        loanRepaymentScheduleTransactionProcessor.handleTransaction(chargesPayment, getCurrency(), chargePaymentInstallments, loanCharges, isExcludeCharges, habLoanRepaymentSchedule, isRepaymentCharge);
        updateLoanSummaryDerivedFields();
    ...
    public void removeLoanCharge(final LoanCharge loanCharge) {
        ...
        if (!loanCharge.isDueAtDisbursement() && loanCharge.isPaidOrPartiallyPaid(loanCurrency())) {
            ...
            loanRepaymentScheduleTransactionProcessor.handleTransaction(getDisbursementDate(), allNonContraTransactionsPostDisbursement,
                    getCurrency(), getRepaymentScheduleInstallments(), charges(), false, null,false);
        }
    ...
    // Changed by Ranjith
    private BigDecimal calculatePerInstallmentTaxAmount(final LoanCharge loanCharge) {
        return calculatePerInstallmentTaxAmount(loanCharge.getChargeCalculation(), loanCharge.getPercentage(),loanCharge.taxPercentage());
    }
    ...
    // Changed by Ranjith
    public BigDecimal calculatePerInstallmentTaxAmount(final ChargeCalculationType calculationType, final BigDecimal percentage, final BigDecimal taxPercentage) {
        Money amount = Money.zero(getCurrency());
        List<LoanRepaymentScheduleInstallment> installments = getRepaymentScheduleInstallments() ;
        for (final LoanRepaymentScheduleInstallment installment : installments) {
            amount = amount.plus(calculateInstallmentTaxAmount(calculationType, percentage, installment,taxPercentage));
        }
        return amount.getAmount();
    }
    ...
    // Changed by Ranjith
    private Money calculateInstallmentTaxAmount(final ChargeCalculationType calculationType, final BigDecimal percentage,
            final LoanRepaymentScheduleInstallment installment,final BigDecimal taxPercentage) {
        Money amount = Money.zero(getCurrency());
        Money percentOf = Money.zero(getCurrency());
        switch (calculationType) {
            case PERCENT_OF_AMOUNT:
        ...
        amount = amount.plus(LoanCharge.percentageOf(percentOf.getAmount(), percentage));
        amount = amount.multipliedBy(taxPercentage.divide(BigDecimal.valueOf(100)));
        return amount;
    }
    ...
    public void updateLoanCharges(final Set<LoanCharge> loanCharges) {
        ...
        for (final LoanCharge loanCharge : loanCharges) {
            ...
            if (charge != null){
           	 // Changed by Ranjith
           	if(loanCharge.getChargeCalculation().isPercentageBased() && loanCharge.isInstalmentFee()){
           		charge.updateInstallmentFee(chargeAmt, loanCharge.getDueLocalDate(), amount, fetchNumberOfInstallmensAfterExceptions(), totalChargeAmt,taxAmount);
           	}
           	else{
    ...
    public void updateLoanSchedule(final LoanScheduleModel modifiedLoanSchedule, AppUser currentUser) {
      ...
      //changed by Ranjith
        BigDecimal interestBrokenChargesDueAtDisbursement=modifiedLoanSchedule.getTotalInterestBrokenChargesCharged();
        updateSummaryWithTotalInterestBrokenChargesDueAtDisbursement(interestBrokenChargesDueAtDisbursement);
        updateTrancheBrokenInterestDetails();
	  ...
	  //changed by Ranjith
	   public void updateTrancheBrokenInterestDetails() {
	   	BigDecimal interestBrokenChargeTranche=BigDecimal.ZERO;
	   	 final RoundingMode roundingMode = MoneyHelper.getRoundingMode();
	         final MathContext mc = new MathContext(8, roundingMode);
	        if(this.disbursementDetails!=null && this.disbursementDetails.size()>0){
	         for (LoanDisbursementDetails disb : this.disbursementDetails) {
	    	 interestBrokenChargeTranche= deriveTrancheInterestBrokenChargesDueAtTimeOfDisbursement(this,disb.principal(),disb.getDisbursementDate(),mc);
	         disb.updateinterestBrokenChargeTranche(interestBrokenChargeTranche);
	         }
	         }
	    }    
    ...
    private void updateLoanSummaryDerivedFields() {
        ...
        if (isNotDisbursed()) {
            ...
            this.summary.updateSummary(this, loanCurrency(), principal, getRepaymentScheduleInstallments(), this.loanSummaryWrapper,
    ...
    public Map<String, Object> loanApplicationModification(final JsonCommand command, final Set<LoanCharge> possiblyModifedLoanCharges,
        ...
        final String amountReturnParamName = "amountReturn";
        if (command.isChangeInBigDecimalParameterNamed(amountReturnParamName, this.amountReturn)) {
            final BigDecimal newValue = command.bigDecimalValueOfParameterNamed(amountReturnParamName);
            this.amountReturn = newValue;
        }
        ...
        if (disbursementID != null) {
            ...
            if (loanDisbursementDetail.actualDisbursementDate() == null) {
                ...
                LoanDisbursementDetails disbursementDetails = new LoanDisbursementDetails(expectedDisbursementDate, actualDisbursementDate,
                        principal,interestBrokenChargeTranche);
    ...
    //changed by Ranjith
    private BigDecimal deriveTrancheInterestBrokenChargesDueAtTimeOfDisbursement(Loan loanApplicationTerms,BigDecimal disbursementAmount,Date disburesementDate1,final MathContext mc) {
        BigDecimal interestBrokenchargesDueAtTimeOfDisbursement = BigDecimal.ZERO;
         BigDecimal interestBrokenDays=BigDecimal.ZERO;
         LocalDate disburesementDate=new LocalDate(disburesementDate1);
        LocalDate interestChargedFromDate=loanApplicationTerms.getInterestChargedFromDate();
          if(interestChargedFromDate!=null){
        	  if(disburesementDate.isBefore(interestChargedFromDate)){
      	        interestBrokenDays = BigDecimal.valueOf(Days.daysBetween(disburesementDate, interestChargedFromDate).getDays());
        	  }
        	  else if(disburesementDate.isAfter(interestChargedFromDate)){
        		  interestChargedFromDate=getNearestRepaymentDate(loanApplicationTerms,disburesementDate);
        		  if(interestChargedFromDate!=null){
        		  interestBrokenDays = BigDecimal.valueOf(Days.daysBetween(disburesementDate, interestChargedFromDate).getDays());
        		  }
        	  }
        	  if(interestBrokenDays.compareTo(BigDecimal.ZERO) < 0){
			  	             	 interestBrokenDays=BigDecimal.ZERO;
      	              }    
          }
       if(loanApplicationTerms.loanRepaymentScheduleDetail.getInterestPeriodFrequencyType().getValue() ==3){
      	    BigDecimal  nominalInterestRate = loanApplicationTerms.loanRepaymentScheduleDetail.getAnnualNominalInterestRate();
      	  Integer daysInYear=loanApplicationTerms.loanRepaymentScheduleDetail.fetchDaysInYearType().getValue();
        if(daysInYear==1){
      	       	   daysInYear =disburesementDate.dayOfYear().getMaximumValue();
         }
      	    BigDecimal daysInYearBigDecimal=new BigDecimal(daysInYear);
      	    BigDecimal percent=new BigDecimal(100);
      	    interestBrokenchargesDueAtTimeOfDisbursement=nominalInterestRate.multiply(disbursementAmount).multiply(interestBrokenDays).divide(daysInYearBigDecimal,mc.getRoundingMode()).divide(percent,mc.getRoundingMode());
          }
          else if(loanApplicationTerms.loanRepaymentScheduleDetail.getInterestPeriodFrequencyType().getValue()==2){
      	     BigDecimal  monthInterestRate = loanApplicationTerms.loanRepaymentScheduleDetail.getNominalInterestRatePerPeriod();
      	     Integer daysInMonth=loanApplicationTerms.loanRepaymentScheduleDetail.fetchDaysInMonthType().getValue();
          if(daysInMonth==1){
      	       daysInMonth =disburesementDate.dayOfMonth().getMaximumValue();
      	   }
      	 BigDecimal daysInMonthBigDecimal=new BigDecimal(daysInMonth);
           BigDecimal percent=new BigDecimal(100);
      	  interestBrokenchargesDueAtTimeOfDisbursement=monthInterestRate.multiply(disbursementAmount).multiply(interestBrokenDays).divide(daysInMonthBigDecimal,mc.getRoundingMode()).divide(percent,mc.getRoundingMode());
      	}
       interestBrokenchargesDueAtTimeOfDisbursement = interestBrokenchargesDueAtTimeOfDisbursement.setScale(0, mc.getRoundingMode());    
      return interestBrokenchargesDueAtTimeOfDisbursement;
     }
    ...
    //changed by Ranjith
    public LocalDate getNearestRepaymentDate(Loan loanApplicationTerms,LocalDate disburesementDate) {
        ...
        for (LoanRepaymentScheduleInstallment installment : this.repaymentScheduleInstallments) {
            if (installment.getDueDate().isAfter(disburesementDate) || installment.getDueDate().isEqual(disburesementDate)) {
              dueDate=installment.getDueDate(); 
              dates.add(dueDate);
            }
        }
       if(dates.size()>0){
    	   dueDate=Collections.min(dates);
         }
       else{
    	   dueDate=null;
       }
    ...
    public void loanApplicationSubmittal(final AppUser currentUser, final LoanScheduleModel loanSchedule,
        ...
        this.amountReturn = loanApplicationTerms.getAmountReturn();
        updateLoanScheduleDependentDerivedFields();
    ...
    public BigDecimal findExistingTransactionInterest( MonetaryCurrency currency) {
        BigDecimal existingInterest=BigDecimal.ZERO;
        List<LoanTransaction> transactions = getLoanTransactions() ;
        for (final LoanTransaction transaction : transactions) {
            if (!transaction.isReversed() && transaction.getTypeOf().getValue()==2) {
            	existingInterest=existingInterest.add(transaction.getInterestPortion(currency).getAmount());
            }
        }
    ...
    private void handleDisbursementTransaction(final LocalDate disbursedOn, final LocalDateTime createdDate, final AppUser currentUser,
        ...
        final Money totalinterestBrokenChargesDueAtDisbursement = this.summary.getTotalInterestBrokenChargesDueAtDisbursement(loanCurrency());
        /**
        ...
        Money disbursmentInterestMoney = Money.zero(getCurrency());
        ...
        if(this.disbursementDetails.size()>0){
	        for (LoanDisbursementDetails disb : this.disbursementDetails) {
	            if (disbursedOn.equals(new LocalDate(disb.getDisbursementDate())) && totalinterestBrokenChargesDueAtDisbursement.isGreaterThanZero()) {
	            	disbursmentInterestMoney=Money.of(getCurrency(),disb.getInterestBrokenTranche());
	            }
	        }
        }
        else{
        	disbursmentInterestMoney=totalinterestBrokenChargesDueAtDisbursement;
        }
        if (disbursentMoney.isGreaterThanZero() || disbursmentInterestMoney.isGreaterThanZero()) {
            final Money zero = Money.zero(getCurrency());
            chargesPayment.updateComponentsAndTotal(zero, disbursmentInterestMoney, disbursentMoney, zero);
            chargesPayment.updateLoan(this);
            addLoanTransaction(chargesPayment) ;
            updateLoanOutstandingBalaces();
        }
    ...
    public ChangedTransactionDetail waiveInterest(final LoanTransaction waiveInterestTransaction,
            ...
            final List<Long> existingReversedTransactionIds, final ScheduleGeneratorDTO scheduleGeneratorDTO, final AppUser currentUser, final boolean isExculdeCharges, final boolean isRepaymentCharge) {
        ...
        final ChangedTransactionDetail changedTransactionDetail = handleRepaymentOrRecoveryOrWaiverTransaction(waiveInterestTransaction,
                loanLifecycleStateMachine, null, scheduleGeneratorDTO, currentUser, isExculdeCharges, isRepaymentCharge);
    ...
    public ChangedTransactionDetail makeRepayment(final LoanTransaction repaymentTransaction,
            ...
            final AppUser currentUser, Boolean isHolidayValidationDone, final boolean isExculdeCharges, final boolean isRepaymentCharge) {
    ...
    public void makeChargePayment(final Long chargeId, final LoanLifecycleStateMachine loanLifecycleStateMachine,
            final List<Long> existingTransactionIds, final List<Long> existingReversedTransactionIds,
            final HolidayDetailDTO holidayDetailDTO, final LoanTransaction paymentTransaction, final Integer installmentNumber, final boolean isExcludeCharges, final boolean isRepaymentCharge) {
        ...
        handleChargePaidTransaction(charge, paymentTransaction, loanLifecycleStateMachine, installmentNumber, isExcludeCharges, null, isRepaymentCharge);
    }
    ...
    private ChangedTransactionDetail handleRepaymentOrRecoveryOrWaiverTransaction(final LoanTransaction loanTransaction,
            ...
            final ScheduleGeneratorDTO scheduleGeneratorDTO, final AppUser currentUser, final boolean isExculdeCharges, final boolean isRepaymentCharge) {
       ...
       if (loanTransaction.isNotRepayment() && loanTransaction.isNotWaiver() && loanTransaction.isNotRecoveryRepayment()) {
        ...
        if(isRepaymentCharge) {
        	if(getRepaymentScheduleInstallments() != null &&
            		!getRepaymentScheduleInstallments().isEmpty()) {
        		Money feesAmount = Money.zero(getCurrency());
        		
            	for(LoanRepaymentScheduleInstallment loanRepaymentScheduleInstallment : getRepaymentScheduleInstallments()) {
            		feesAmount = feesAmount
                            .plus(loanRepaymentScheduleInstallment.getFeeChargesCharged(getCurrency()).minus(loanRepaymentScheduleInstallment.getFeeChargesPaid(getCurrency())))
                            .plus(loanRepaymentScheduleInstallment.getPenaltyChargesCharged(getCurrency())).minus(loanRepaymentScheduleInstallment.getPenaltyChargesPaid(getCurrency()));
            		
            	}
            	if(loanTransaction.getAmount(getCurrency()).isGreaterThan(feesAmount)) {
            		 final String errorMessage = "The amount of transation greater then charges : ";
            				 throw new InvalidLoanStateTransitionException("transaction", "amount.exceeds.outstanding.charges", errorMessage);
            	}
            }
        }
        ...
        if (isTransactionChronologicallyLatest && adjustedTransaction == null
            ...
            loanRepaymentScheduleTransactionProcessor.handleTransaction(loanTransaction, getCurrency(), getRepaymentScheduleInstallments(),
                    charges(), isExculdeCharges, null, isRepaymentCharge);
            reprocess = false;
            if (this.repaymentScheduleDetail().isInterestRecalculationEnabled()) {
        ...
        if (reprocess) {
            ...
            final List<LoanTransaction> allNonContraTransactionsPostDisbursement = retreiveListOfTransactionsPostDisbursement();
            changedTransactionDetail = loanRepaymentScheduleTransactionProcessor.handleTransaction(getDisbursementDate(),
                    allNonContraTransactionsPostDisbursement, getCurrency(), getRepaymentScheduleInstallments(), charges(), isExculdeCharges, null,isRepaymentCharge);
            for (final Map.Entry<Long, LoanTransaction> mapEntry : changedTransactionDetail.getNewTransactionMappings().entrySet()) {
                mapEntry.getValue().updateLoan(this);
            }
            /***
    ...
    public ChangedTransactionDetail adjustExistingTransaction(final LoanTransaction newTransactionDetail,
        ...
        if (newTransactionDetail.isRepayment() || newTransactionDetail.isInterestWaiver()) {
            changedTransactionDetail = handleRepaymentOrRecoveryOrWaiverTransaction(newTransactionDetail, loanLifecycleStateMachine,
                    transactionForAdjustment, scheduleGeneratorDTO, currentUser, isExculdeCharges, isRepaymentCharge);
        }
    ...
    public List<LoanInstallmentCharge> generateInstallmentLoanCharges(final LoanCharge loanCharge, final MathContext mc) {
        ...
        if (loanCharge.isInstalmentFee()) {
            ...
            for (final LoanRepaymentScheduleInstallment installment : installments) {
                ...
                if (loanCharge.getChargeCalculation().isFlat()) {
                	// Changed by Mohan
                	taxAmount = loanCharge.amountOrPercentage().multiply(loanCharge.taxPercentage()).divide(BigDecimal.valueOf(100));
                    ...
                    amount = calculateInstallmentChargeAmount(loanCharge.getChargeCalculation(), loanCharge.getPercentage(), installment)
                            .getAmount();
                    // Changed by Ranjith
                    taxAmount = calculateInstallmentTaxAmount(loanCharge.getChargeCalculation(), loanCharge.getPercentage(), installment,loanCharge.taxPercentage())
                            .getAmount();
                    // Changed by Mohan

                   // taxAmount = amount.multiply(loanCharge.taxPercentage()).divide(BigDecimal.valueOf(100));
               } 
                amount = amount.setScale(0, mc.getRoundingMode());
             // Changed by Mohan
                taxAmount = taxAmount.setScale(0, mc.getRoundingMode());
                final LoanInstallmentCharge loanInstallmentCharge = new LoanInstallmentCharge(amount.add(taxAmount), loanCharge, installment,taxAmount,loanCharge.taxPercentage());
    ...
    public LoanCharge fetchLoanChargesById(Long id) {
        ...
        for (LoanCharge loanCharge : this.charges) {
        	if(loanCharge.getId()!=null && id!=null){
    ...
    public void processPostDisbursementTransactions() {
        ...
        if (allNonContraTransactionsPostDisbursement.size() > 0) {
            ...
            loanRepaymentScheduleTransactionProcessor.handleTransaction(getDisbursementDate(), copyTransactions, getCurrency(),
                    getRepaymentScheduleInstallments(), charges(), false, null,false);
    ...
    public LoanApplicationTerms constructLoanApplicationTerms(final ScheduleGeneratorDTO scheduleGeneratorDTO) {
        ...
        final LoanApplicationTerms loanApplicationTerms = LoanApplicationTerms.assembleFrom(scheduleGeneratorDTO.getApplicationCurrency(),
                ...
                this.maxOutstandingLoanBalance, interestChargedFromDate, amountReturn, this.loanProduct.getPrincipalThresholdForLastInstallment(),
    ...
    public List<DisbursementData> getDisbursmentData() {
        ...
        while (iterator.hasNext()) {
            ...
            disbursementData.add(new DisbursementData(loanDisbursementDetails.getId(), expectedDisbursementDate, actualDisbursementDate,
                    loanDisbursementDetails.principal(), null, null, waivedChargeAmount, null));
        }
    ...
    public LoanApplicationTerms getLoanApplicationTerms(final ApplicationCurrency applicationCurrency,
        ...
        return LoanApplicationTerms.assembleFrom(applicationCurrency, loanTermFrequency, loanTermPeriodFrequencyType, nthDayType,
                ...
                maxOutstandingBalance, interestChargedFromDate, amountReturn, this.loanProduct.getPrincipalThresholdForLastInstallment(),
                this.loanProduct.getInstallmentAmountInMultiplesOf(), recalculationFrequencyType, restCalendarInstance, compoundingMethod,
                compoundingCalendarInstance, compoundingFrequencyType, this.loanProduct.preCloseInterestCalculationStrategy(),
    ...
    private boolean isLessInterest(LocalDate transactionDate,Money interestPayableManual,Money interestPayableActual){
        boolean isLessInterest = false;
       // final MonetaryCurrency currency = getCurrency();
       // Money totalInterest = Money.zero(currency);
       /* for (final LoanRepaymentScheduleInstallment installment : this.repaymentScheduleInstallments) {
        	if (installment.getDueDate().isBefore(transactionDate)){
        		Money interestRemaining=installment.getInterestCharged(currency).minus(
        				installment.getInterestPaid(currency).plus(installment.getInterestWaived(currency)).plus
        						(installment.getInterestWrittenOff(currency)));
        		totalInterest=totalInterest.plus(interestRemaining);	
        	}
        }*/

        if(interestPayableManual.isLessThan(interestPayableActual)){
			isLessInterest = true;
		}
        return isLessInterest;
    }

    
    public void updateInstallmentsPostDate(LocalDate transactionDate ,Money interestPayable ,Money interestPayableActual,Money interestPayableManual) {
        List<LoanRepaymentScheduleInstallment> newInstallments = new ArrayList<>(this.repaymentScheduleInstallments);
        int repaymentActualSize=newInstallments.size();
        final MonetaryCurrency currency = getCurrency();
        ...
        int i=1;
        //changed by Ranjith
        if(repaymentActualSize!=1){
        if (isLessInterest(transactionDate,interestPayableManual,interestPayableActual)) {
            final String defaultUserMessage = "The loan with interest less than actual interest cannot be foreclosed.";
            throw new LoanForeclosureException("loan.with.interest.less.than.actual.interest.cannot.be.foreclosured", defaultUserMessage,
                    getId());
          }
        }
        for (final LoanRepaymentScheduleInstallment installment : this.repaymentScheduleInstallments) {
       	
       	 if (newInstallments.size() == 1 ) { //size 1
    		 balances[0]=interestPayable;
    	 }

        	
            if (!installment.getDueDate().isBefore(transactionDate)) { //after
        	...
        	//changed by Ranjith
       	 if (installment.getDueDate().isBefore(transactionDate) && repaymentActualSize!=1) { //before multiple
       		Money interestRemaining=installment.getInterestCharged(currency).minus(
       				installment.getInterestPaid(currency).plus(installment.getInterestWaived(currency)).plus
       						(installment.getInterestWrittenOff(currency)));
       		if(interestPayable.isGreaterThanOrEqualTo(interestRemaining)){
       		interestPayable=interestPayable.minus(interestRemaining);
       		}
       		if(interestPayable.isLessThan(interestRemaining)){
                /*totalPrincipal = totalPrincipal.plus(installment.getPrincipal(currency));
                newInstallments.remove(installment);*/
                balances[0]=interestPayable;
       		}
       		if(interestRemaining.isZero()){
       		interestPayable=interestPayable.minus(installment.getInterestCharged(currency));
       		balances[0]=interestPayable;
       		}
       	}  
       	...
       	checkAfter = false;
       	 ...
       	 /*if (newInstallments.size() > 1 && installment.getDueDate().isBefore(transactionDate)) { //size many
                totalPrincipal = totalPrincipal.plus(installment.getPrincipal(currency));
                if(newInstallments.size()==i){
               	 installment.updateInterestCharged(interestPayable.getAmount());
                }
                if (installment.getDueDate().isEqual(transactionDate)) {
					isInterestComponent = false;
				}
		...
		if (balances[0].plus(interestPayable).plus(balances[1]).plus(balances[2]).isGreaterThanZero()) {
