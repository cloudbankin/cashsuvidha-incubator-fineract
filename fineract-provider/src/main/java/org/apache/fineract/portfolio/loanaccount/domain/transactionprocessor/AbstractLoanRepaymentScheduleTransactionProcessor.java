...
import org.apache.fineract.portfolio.loanaccount.domain.HabLoanRepaymentSchedule;
...
public abstract class AbstractLoanRepaymentScheduleTransactionProcessor implements LoanRepaymentScheduleTransactionProcessor {
    ...
    public ChangedTransactionDetail handleTransaction(final LocalDate disbursementDate,
            ...
            final List<LoanRepaymentScheduleInstallment> installments, final Set<LoanCharge> charges, final boolean isExculdeCharges, final Set<HabLoanRepaymentSchedule> habLoanRepaymentSchedule, final boolean isRepaymentCharge) {
            ...
            if (loanTransaction.isChargePayment()) {
                ...
                for (LoanChargePaidDetail chargePaidDetail : chargePaidDetails) {
                    ...
                    unprocessed = handleTransactionAndCharges(loanTransaction, currency, processInstallments, transferCharges, processAmt,
                            chargePaidDetail.isFeeCharge(), isExculdeCharges, habLoanRepaymentSchedule, isRepaymentCharge);
        ...
        for (final LoanTransaction loanTransaction : transactionstoBeProcessed) {
            ...
            if (loanTransaction.isRepayment() || loanTransaction.isInterestWaiver() || loanTransaction.isRecoveryRepayment()) {
            	...
            	Set<HabLoanRepaymentSchedule> habLoanRepaymentSchedule2 = new HashSet<HabLoanRepaymentSchedule>();
            	
            	if(loanTransaction.getHabLoanRepaymentSchedule() != null &&
                		!loanTransaction.getHabLoanRepaymentSchedule().isEmpty()) {
            		habLoanRepaymentSchedule2 = loanTransaction.getHabLoanRepaymentSchedule();
                }else {
                	habLoanRepaymentSchedule2 = null;
                }
            	
                // pass through for new transactions
                if (loanTransaction.getId() == null) {
                    handleTransaction(loanTransaction, currency, installments, charges, isExculdeCharges, habLoanRepaymentSchedule2, isRepaymentCharge);
                    ...
                    handleTransaction(newLoanTransaction, currency, installments, charges, isExculdeCharges,habLoanRepaymentSchedule2, isRepaymentCharge);
    ...
    public void handleTransaction(final LoanTransaction loanTransaction, final MonetaryCurrency currency,
        ...
        handleTransaction(loanTransaction, currency, installments, charges, amountToProcess, isChargeAmount, isExculdeCharges, habLoanRepaymentSchedule, isRepaymentCharge);
    ...
    private void handleTransaction(final LoanTransaction loanTransaction, final MonetaryCurrency currency,
            ...
            final boolean isFeeCharge, final boolean isExculdeCharges, final Set<HabLoanRepaymentSchedule> habLoanRepaymentSchedule, final boolean isRepaymentCharge) {

        Money transactionAmountUnprocessed = handleTransactionAndCharges(loanTransaction, currency, installments, charges,
                chargeAmountToProcess, isFeeCharge, isExculdeCharges, habLoanRepaymentSchedule, isRepaymentCharge);
    ...
    private Money handleTransactionAndCharges(final LoanTransaction loanTransaction, final MonetaryCurrency currency,
        ...
        Money transactionAmountUnprocessed = processTransaction(loanTransaction, currency, installments, chargeAmountToProcess, isExculdeCharges, habLoanRepaymentSchedule,  isRepaymentCharge);
    ...
    private Money processTransaction(final LoanTransaction loanTransaction, final MonetaryCurrency currency,
            final List<LoanRepaymentScheduleInstallment> installments, Money amountToProcess, final boolean isExculdeCharges, final Set<HabLoanRepaymentSchedule> habLoanRepaymentSchedules, final boolean isRepaymentCharge) {
            ...
            if (transactionAmountUnprocessed.isGreaterThanZero()) {
                if (currentInstallment.isNotFullyPaidOff()) {
                	...
                	if(loanTransaction.isRepayment()) {
                		if(habLoanRepaymentSchedules != null &&
                    			!habLoanRepaymentSchedules.isEmpty()) {
                    		for(HabLoanRepaymentSchedule habLoanRepaymentSchedule : habLoanRepaymentSchedules) {
                    			if(habLoanRepaymentSchedule.getLoanRepaymentScheduleInstallment().getId().equals(currentInstallment.getId())) {
                    				isContinue = false;
                    			}
                    		}
                    		if(isContinue) {
                    		...
                    		if(!isRepaymentCharge) {
    	                		if(!currentInstallment.getPrincipalCompleted(currency).getAmount().equals(0) &&
    		            				!currentInstallment.getInterestPaid(currency).getAmount().equals(0) &&
    		            				currentInstallment.getPrincipalCompleted(currency).getAmount().equals(currentInstallment.getPrincipal(currency).getAmount()) &&
    		            				currentInstallment.getInterestPaid(currency).getAmount().equals(currentInstallment.getInterestCharged(currency).getAmount())) {
    	               				continue;
    	                		}
                    		}
                    	}
                    ...
                    if (isTransactionInAdvanceOfInstallment(installmentIndex, installments, transactionDate, transactionAmountUnprocessed)) {
                        transactionAmountUnprocessed = handleTransactionThatIsPaymentInAdvanceOfInstallment(currentInstallment,
                                installments, loanTransaction, transactionDate, transactionAmountUnprocessed, transactionMappings, isExculdeCharges, habLoanRepaymentSchedules, isRepaymentCharge);
                        ...
                        transactionAmountUnprocessed = handleTransactionThatIsALateRepaymentOfInstallment(currentInstallment, installments,
                                loanTransaction, transactionAmountUnprocessed, transactionMappings,isExculdeCharges, habLoanRepaymentSchedules, isRepaymentCharge);
                    } else {
        ...
        loanTransaction.updateLoanTransactionToHabRepaymentSchedule(loanTransaction, isExculdeCharges, transactionMappings, isRepaymentCharge);
        return transactionAmountUnprocessed;
    }
    ...
    protected abstract Money handleTransactionThatIsALateRepaymentOfInstallment(final LoanRepaymentScheduleInstallment currentInstallment,
            ...
            final Money transactionAmountUnprocessed, final List<LoanTransactionToRepaymentScheduleMapping> transactionMappings, final boolean isExculdeCharges, final Set<HabLoanRepaymentSchedule> habLoanRepaymentSchedule
            , final boolean isRepaymentCharge);
    ...
    protected abstract Money handleTransactionThatIsPaymentInAdvanceOfInstallment(
            ...
            final List<LoanTransactionToRepaymentScheduleMapping> transactionMappings, final boolean isExcludeCharges, final Set<HabLoanRepaymentSchedule> habLoanRepaymentSchedule, final boolean isRepaymentCharge);
    ...
    protected abstract Money handleTransactionThatIsOnTimePaymentOfInstallment(final LoanRepaymentScheduleInstallment currentInstallment,
            ...
            final List<LoanTransactionToRepaymentScheduleMapping> transactionMappings, final boolean isExculdeCharges, final Set<HabLoanRepaymentSchedule> habLoanRepaymentSchedule, final boolean isRepaymentCharge);
    ...
    public Money handleRepaymentSchedule(final List<LoanTransaction> transactionsPostDisbursement, final MonetaryCurrency currency,
        ...
        for (final LoanTransaction loanTransaction : transactionsPostDisbursement) {
            ...
            if (loanTransaction.isInterestWaiver()) {
                processTransaction(loanTransaction, currency, installments, amountToProcess, false, null,false);
            } else {
                unProcessed = processTransaction(loanTransaction, currency, installments, amountToProcess, false, null,false);
            }
        }
        return unProcessed;
    }
