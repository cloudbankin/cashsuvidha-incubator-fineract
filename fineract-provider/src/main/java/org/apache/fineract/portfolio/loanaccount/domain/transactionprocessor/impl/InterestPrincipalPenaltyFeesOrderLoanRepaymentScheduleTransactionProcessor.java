...
public class InterestPrincipalPenaltyFeesOrderLoanRepaymentScheduleTransactionProcessor extends
    ...
    protected Money handleTransactionThatIsPaymentInAdvanceOfInstallment(final LoanRepaymentScheduleInstallment currentInstallment,
            ...
            List<LoanTransactionToRepaymentScheduleMapping> transactionMappings, final boolean isExculdeCharges, final Set<HabLoanRepaymentSchedule> habLoanRepaymentSchedule, final boolean isRepaymentCharge) {

        return handleTransactionThatIsOnTimePaymentOfInstallment(currentInstallment, loanTransaction, paymentInAdvance, transactionMappings, isExculdeCharges, habLoanRepaymentSchedule, isRepaymentCharge);
    }
    ...
    protected Money handleTransactionThatIsALateRepaymentOfInstallment(final LoanRepaymentScheduleInstallment currentInstallment,
            final List<LoanRepaymentScheduleInstallment> installments, final LoanTransaction loanTransaction,
            final Money transactionAmountUnprocessed, List<LoanTransactionToRepaymentScheduleMapping> transactionMappings, final boolean isExculdeCharges, final Set<HabLoanRepaymentSchedule> habLoanRepaymentSchedule, final boolean isRepaymentCharge) {

        return handleTransactionThatIsOnTimePaymentOfInstallment(currentInstallment, loanTransaction, transactionAmountUnprocessed,
                transactionMappings, isExculdeCharges, habLoanRepaymentSchedule, isRepaymentCharge);
    }
    ...
    protected Money handleTransactionThatIsOnTimePaymentOfInstallment(final LoanRepaymentScheduleInstallment currentInstallment,
            ...
            List<LoanTransactionToRepaymentScheduleMapping> transactionMappings, final boolean isExculdeCharges, final Set<HabLoanRepaymentSchedule> habLoanRepaymentSchedules, final boolean isRepaymentCharge) {
    	boolean isHabLoanEmpty = false;
    	if(habLoanRepaymentSchedules == null) {
    		isHabLoanEmpty = false;
    	}else if(habLoanRepaymentSchedules.isEmpty()){
    		isHabLoanEmpty = true;
    	}
        ...
        if (loanTransaction.isChargesWaiver()) {
            ...
            loanTransaction.updateComponents(principalPortion, interestPortion, feeChargesPortion, penaltyChargesPortion);
        }else if(isRepaymentCharge && 
        		habLoanRepaymentSchedules == null) {
        	penaltyChargesPortion = currentInstallment.payPenaltyChargesComponent(transactionDate, transactionAmountRemaining);
            transactionAmountRemaining = transactionAmountRemaining.minus(penaltyChargesPortion);
            ...
            loanTransaction.updateComponents(principalPortion, interestPortion, feeChargesPortion, penaltyChargesPortion);
        }else if(loanTransaction.isRepayment()) {
             if(habLoanRepaymentSchedules != null &&
               	...
               	for(HabLoanRepaymentSchedule habLoanRepaymentSchedule : habLoanRepaymentSchedules) {
               		if(habLoanRepaymentSchedule.getLoanRepaymentScheduleInstallment().getId().equals(currentInstallment.getId())) {
	           			if(!habLoanRepaymentSchedule.isRepaymentCharges()){
               			...
               			if(!habLoanRepaymentSchedule.isPaymentCharges()) {
	           				penaltyChargesPortion = currentInstallment.payPenaltyChargesComponent(transactionDate, transactionAmountRemaining);
	                           transactionAmountRemaining = transactionAmountRemaining.minus(penaltyChargesPortion);
