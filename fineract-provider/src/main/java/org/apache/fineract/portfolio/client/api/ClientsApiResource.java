...
public class ClientsApiResource {
  ...
  // collection Efficiency Newly added api call 
    @GET
    @Path("/collectionEfficiencyReport")
    @Consumes({ MediaType.APPLICATION_JSON })
    @Produces({ MediaType.APPLICATION_OCTET_STREAM })
    public Response retrieveLoanSchedule(@QueryParam("fromDate") final String fromDate, @QueryParam("toDate") final String toDate,@QueryParam("office") final Long office, @Context final UriInfo uriInfo) {

        this.context.authenticatedUser().validateHasReadPermission(ClientApiConstants.CLIENT_RESOURCE_NAME);
        Workbook result =this.clientReadPlatformService.retrieveCollectionEfficiencyreport(fromDate, toDate, office);
		...
		try 
		{
	        ...
	        response.header("Content-Disposition", "attachment; filename=\"" + "Collection_Efficiency_Report.xls"+ "\"");
