...
public class LoanArrearsAgingServiceImpl implements LoanArrearsAgingService, BusinessEventListner {
    ...
    @Autowired
    public LoanArrearsAgingServiceImpl(final RoutingDataSource dataSource, final BusinessEventNotifierService businessEventNotifierService,final SmsProcessingService smsProcessingService,final MailProcessingService mailProcessingService) {
    ...
    public void updateLoanArrearsAgeingDetails() {
        ...
        //Job to send sms to overdue starts
        try {
        	LoanArrearTemplateMapper mapper = new LoanArrearTemplateMapper();
            String sql = mapper.schema();
//            int overdueAfterDaysCount = Integer.valueOf(env.getProperty("sms.overdue.after.days"));
            
            String[] ary = env.getProperty("sms.overdue.after.days").split(",");
            ...
            boolean isActive = Boolean.valueOf(env.getProperty("sms.overdue.isActive"));
            ...
            for(LoanDueData arrearData : loanTransactionData) {
            	if(CommonMethodsUtil.isNotBlank(arrearData.getCurrency())&&CommonMethodsUtil.isNotBlank(arrearData.getCurrency())&&CommonMethodsUtil.isNotBlank(arrearData.getDueAmount())) {
                    String templateParamString = "templateParameters[A]="+arrearData.getClientName()+"&templateParameters[B]="+arrearData.getCurrency()+"&templateParameters[C]="+arrearData.getDueAmount().setScale(2, BigDecimal.ROUND_HALF_EVEN).toString()+"&templateParameters[D]="+arrearData.getDueSinceDate().toString("dd/MM/yyyy");
                    SMSDataVO smsdata = new SMSDataVO("overdue", arrearData.getClientMobileNumber(), templateParamString);
        	...
        	logger.error(ThreadLocalContextUtil.getTenant().getName() + ": Exception in sending overdue reminder sms: " + e.getMessage());
        }

        //Job to send sms to overdue ends
        
        //Job to send sms to remind repayments starts
        try {
        	tablename = String.valueOf(env.getProperty("mail.clienttable.datatablename"));
            columnname = String.valueOf(env.getProperty("mail.clienttable.datatablecolumnname"));
            ...
            if(CommonMethodsUtil.isNotBlank(loanTransactionData)) {
            for(LoanDueData arrearData : loanTransactionData) {
            	if(CommonMethodsUtil.isNotBlank(arrearData.getClientMobileNumber())&&CommonMethodsUtil.isNotBlank(arrearData.getClientName())&&CommonMethodsUtil.isNotBlank(arrearData.getCurrency())&&CommonMethodsUtil.isNotBlank(arrearData.getDueAmount())) {
                    String templateParamString = arrearData.getClientName()+","+arrearData.getCurrency()+","+arrearData.getDueAmount().setScale(2, BigDecimal.ROUND_HALF_EVEN).toString()+","+arrearData.getDueSinceDate().toString("dd/MM/yyyy");
                    SMSDataVONimbus smsdata = new SMSDataVONimbus("repaymentReminder", arrearData.getClientMobileNumber(), templateParamString,"repaymentReminderContent");
            ...
            boolean isActivemail = Boolean.valueOf(env.getProperty("mail.repaymentReminder.isActive"));
            if(isActivemail && CommonMethodsUtil.isNotBlank(ids)) {
            final Collection<LoanDueData> loanTransactionData = this.namedJdbctemplate.query(sql, parameters,mapper);
            if(CommonMethodsUtil.isNotBlank(loanTransactionData)) {
            for(LoanDueData arrearData : loanTransactionData) {
            	if(CommonMethodsUtil.isNotBlank(arrearData.getClientemailid())&&CommonMethodsUtil.isNotBlank(arrearData.getClientName())&&CommonMethodsUtil.isNotBlank(arrearData.getCurrency())&&CommonMethodsUtil.isNotBlank(arrearData.getDueAmount())) {
                    String templateParamString = arrearData.getClientName()+","+arrearData.getCurrency()+","+arrearData.getDueAmount().setScale(2, BigDecimal.ROUND_HALF_EVEN).toString()+","+arrearData.getDueSinceDate().toString("dd/MM/yyyy");
                    
                    MailDataVO maildata = new MailDataVO("subject.repaymentReminder", arrearData.getClientemailid(), templateParamString,"repaymentReminderContent","","");
                    this.mailProcessingService.sendMail(maildata);
    ...
    @SuppressWarnings("unused")
	private static final class LoanArrearTemplateMapper implements RowMapper<LoanDueData> {
...
//            sqlBuilder.append("SELECT ifnull(cur.display_symbol, ml.currency_code) as currency,");
            ...
            sqlBuilder.append("laa.overdue_since_date_derived AS overdue_date,");
            sqlBuilder.append("ifnull(laa.total_overdue_derived,0) AS overdue_amount ");
            sqlBuilder.append(" FROM m_loan_arrears_aging laa ");
            sqlBuilder.append(" INNER JOIN m_loan ml ON ml.id = laa.loan_id ");
...
//            sqlBuilder.append(" left join m_currency cur on cur.code = ml.currency_code ");
            sqlBuilder.append(" WHERE ml.loan_status_id=300 ");
            sqlBuilder.append(" and mc.mobile_no IS NOT NULL ");
            sqlBuilder.append(" and DATEDIFF(CURDATE(), laa.overdue_since_date_derived) in (:ids) ");
        ...
        public LoanDueData mapRow(ResultSet rs, int rowNum) throws SQLException {
            ...
            final BigDecimal overdueAmount = JdbcSupport.getBigDecimalDefaultToZeroIfNull(rs, "overdue_amount");
            final LocalDate date = JdbcSupport.getLocalDate(rs, "overdue_date");
            final String clienteMailid =rs.getString("emailid");
            
           
            return new LoanDueData(currency,loanAccountNumber,clientAccountNumber,clientName,clientMobileNumber,overdueAmount,date,clienteMailid);
        }
}
	...
	private static final class LoanRepaymentTemplateMapper implements RowMapper<LoanDueData> {
        ...
        public String schema() {
            ...
            sqlBuilder.append("mc.display_name AS client_name,");
            sqlBuilder.append("mc.mobile_no as mobile_no,");
            if(CommonMethodsUtil.isNotBlank(columnname))
            sqlBuilder.append("tn."+columnname+" as emailid,");
            ...
            sqlBuilder.append(" INNER JOIN m_client mc ON mc.id=ml.client_id ");
            sqlBuilder.append(" INNER JOIN "+tablename+" tn ON mc.id=tn.client_id ");
...
//            sqlBuilder.append(" left join m_currency cur on cur.code = ml.currency_code ");
        ...
        public LoanDueData mapRow(ResultSet rs, int rowNum) throws SQLException {
            ...
            return new LoanDueData(currency,loanAccountNumber,clientAccountNumber,clientName,clientMobileNumber,dueAmount,date,clienteMailid);
        }
}
}
