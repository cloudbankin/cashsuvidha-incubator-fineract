...
public class TransferWritePlatformServiceJpaRepositoryImpl implements TransferWritePlatformService {
    ...
    private void handleClientTransferLifecycleEvent(final Client client, final Office destinationOffice,
        ...
        final Date transactionDate = jsonCommand.DateValueOfParameterNamed(TransferApiConstants.transactionDate);
        if (staffId != null) {
            ...
            for (final Loan loan : this.loanRepositoryWrapper.findLoanByClientId(client.getId())) {
                ...
                if (loan.isDisbursed() && !loan.isClosed()) {
                    switch (transferEventType) {
                        case ACCEPTANCE:
                            this.loanWritePlatformService.acceptLoanTransfer(loan, client.getOfficeJoiningLocalDate(),
                                    destinationOffice, staff);
                        break;
                        case PROPOSAL:
                            this.loanWritePlatformService.initiateLoanTransfer(loan, LocalDate.fromDateFields(transactionDate));
                        break;
                        case REJECTION:
...
//                client.updateOfficeJoiningDate(todaysDate);
            ...
            case PROPOSAL:
                ...
                //changed By Ranjith
                client.updateOfficeJoiningDate(transactionDate);
            break;
            case REJECTION:
            ...
            case WITHDRAWAL:
                ...
                client.updateTransferToOffice(null);
                //changed By Ranjith
                client.updateOfficeJoiningDate(client.getActivationDate());
        }
