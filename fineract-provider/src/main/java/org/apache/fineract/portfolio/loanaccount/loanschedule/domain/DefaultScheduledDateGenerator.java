...
public class DefaultScheduledDateGenerator implements ScheduledDateGenerator {
    ...
    @Override
    public List<LocalDate> getAllRepaymentDates(final LoanApplicationTerms loanApplicationTerms, final HolidayDetailDTO holidayDetailDTO) {
        final int numberOfRepayments = loanApplicationTerms.getNumberOfRepayments();
        List<LocalDate> dates = new ArrayList<LocalDate>();
        LocalDate RepaymentDates = loanApplicationTerms.getExpectedDisbursementDate();
        boolean isFirstRepayment = true;
        for (int repaymentPeriod = 1; repaymentPeriod <= numberOfRepayments; repaymentPeriod++) {
        	RepaymentDates = generateNextRepaymentDate(RepaymentDates, loanApplicationTerms, isFirstRepayment);
        	RepaymentDates = adjustRepaymentDate(RepaymentDates, loanApplicationTerms, holidayDetailDTO).getChangedScheduleDate();
            isFirstRepayment = false;
            dates.add(RepaymentDates);
        }
